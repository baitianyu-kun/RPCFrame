...
namespace mrpc {
    ...
    void TimerFDEvent::deleteTimerEvent(TimerEventInfo::ptr time_event) {
        ...
        for (iter = begin; iter != end; ++iter) {
            if (iter->second == time_event) {
                DEBUGLOG("success delete TimerEvent at arrive time %lld, name %s", time_event->getArriveTime(),
                         time_event->name.c_str());
    ...
    void TimerFDEvent::resetTimerEvent(TimerEventInfo::ptr time_event) {
        deleteTimerEvent(time_event);
        auto t2 = time_event;
        t2->setArriveTime();
        time_event.reset();
//        time_event->setArriveTime();
        addTimerEvent(t2);
    }
}
