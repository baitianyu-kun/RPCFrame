...
namespace mrpc {
    ...
    bool MPbProtocolParser::parse(std::string &str) {
        ...
        auto m_pb_protocol = std::static_pointer_cast<MPbProtocol>(m_protocol);
        const char *data = str.data();
        size_t offset = 0;
        // 检查字符串长度是否足够解析最小的头部信息
        if (str.length() < sizeof(m_pb_protocol->m_magic) + sizeof(m_pb_protocol->m_type) + 2 * sizeof(uint32_t)) {
            return false; // 数据不足，无法解析
        }
        // 解析 magic
        memcpy(&m_pb_protocol->m_magic, data + offset, sizeof(m_pb_protocol->m_magic));
        offset += sizeof(m_pb_protocol->m_magic);
        if (m_pb_protocol->m_magic != MAGIC) {
            return false; // 检查魔数有效性
        }
        // 解析 type
        memcpy(&m_pb_protocol->m_type, data + offset, sizeof(m_pb_protocol->m_type));
        offset += sizeof(m_pb_protocol->m_type);
        ...
        // 解析 msg_id
        m_pb_protocol->m_msg_id.assign(data + offset, msg_id_len);
        offset += msg_id_len;
        ...
        // 解析 content
        m_pb_protocol->m_body.assign(data + offset, content_len);
        splitStrToMap(m_pb_protocol->m_body, g_CRLF, ":", m_pb_protocol->m_body_data_map);
        offset += content_len;
        // 检查是否解析了所有数据
