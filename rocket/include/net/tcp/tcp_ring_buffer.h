...
namespace rocket {
    class TCPRingBuffer {
    public:
        using tcp_ring_buff_sptr_t_ = std::shared_ptr<TCPRingBuffer>;
    ...
    public:
        ...
        void writeToBuffer(char *buff, int size);

        void writeToBuffer(const char *buff, int size);

        int getReadIndex1() const { return m_read_idx; }

        int getWriteIndex1() const { return m_write_idx; }

        int readAbleSize() const { return RingBuffNumItems(); }

        // 环形会自动覆盖，所以最多可以写入这么多个，用来给read需要指定读取大小来使用
        int writeAbleSize() const { return m_max_size - 1; };
