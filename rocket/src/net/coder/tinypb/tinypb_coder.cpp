...
namespace rocket {
    ...
    void TinyPBCoder::decode(std::vector<AbstractProtocol::abstract_pro_sptr_t_> &out_messages,
        ...
        while (true) {
            ...
            std::vector<char> tmp;
            in_buffer->readFromBuffer(tmp, in_buffer->readAbleSize());

            int start_index = 0;
            int end_index = -1;
            int pk_len = 0; // 整包长度
            bool parse_success = false;
            // 因为在tcp connection的时候，read完成后会move write index，所以write index的位置就是停止的位置
            int index = 0;
            for (index = start_index; index < tmp.size(); index++) {
