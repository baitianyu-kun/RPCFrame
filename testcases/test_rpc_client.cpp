...
int main() {
    ...
    std::string request_str = "POST /discovery HTTP/1.0\r\n"
                              ...
                              "\r\n"
                              "msg_id:1234";
    HTTPRequestParser::ptr requestParser;
    ...
    client->connect([client, request, request_body_data_map]() {
        INFOLOG("%s | success connect. peer addr [%s], local addr[%s]",
                request_body_data_map["msg_id"].c_str(),
                client->getPeerAddr()->toString().c_str(),
                client->getLocalAddr()->toString().c_str());

        client->sendRequest(request, [client, request, request_body_data_map](HTTPRequest::ptr msg) {

            INFOLOG("%s | success send rpc request. call method name [%s], peer addr [%s], local addr[%s]",
                    response_body_data_map["msg_id"].c_str(),
                    request->m_request_body.c_str(),
                    client->getPeerAddr()->toString().c_str(),
                    client->getLocalAddr()->toString().c_str());

            client->recvResponse(response_body_data_map["msg_id"], [client, request](HTTPResponse::ptr msg) {
